...
- Create a `EasyDISettings` scriptable object and put it in a _Resources_ folder. Assign the application-lifetime-scope prefab to it, and the session-lifetime-scope and game-lifetime-scope prefabs if you created them.
...
- Session-lifetime-scope and game-lifetime-scope need to be created and destroyed at runtime manually, in line with a session and a game's respective start and end. You can do this in a few ways:
  ...
  - Instantiate the prefabs referenced in `EasyDISettings.SessionLifetimeScope` and `EasyDISettings.GameLifetimeScope`.
